...
export const Stake = ({ job }) => {
  ...
  const { enterLaborGrounds, nfts } = useContext(ContractContext);
  ...
  const { open: isPitsValidated } = useStakedInPits();
    ...
    <>
      {!eligibleSmols || eligibleSmols?.length === 0 ? (
        ...
        <div>
          ...
          <div
            className={`grid max-h-[35vh] grid-cols-2 gap-2 overflow-y-scroll xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-[${min(
              eligibleSmols.length,
              5,
            )}]`}
          >
          ...
          {selected?.length > 0 && (
            <button
              ...
              className="btn mx-auto flex rounded-xl border-smol-brown-light bg-[#8a0303] px-5 py-3 text-xs uppercase"
            >
