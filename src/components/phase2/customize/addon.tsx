...
export function Addon({
  ...
  const {
    data: addonPrice,
    isLoading: isLoadingAddonPrice,
    error: errorFetchingAddonPrice,
  } = useContractRead({
    address: ADDONS_CONTROLLER_ADDRESS[chainId],
    abi: AddonsController__factory.abi,
    args: [addon.id],
  ...
  const {
    hasCouncilPass,
    isLoading: isLoadingHasCouncilPass,
    error: errorFetchingCouncilPass,
  } = useHasCouncilPass();
  ...
  const { bonesPrice, magicPrice, isMagicTokenB } = useMemo(() => {
    if (addonPrice) {
      ...
      return {
        bonesPrice,
        magicPrice,
        isMagicTokenB: tokenB.toLowerCase() === MAGIC_ADDRESS[chainId].toLowerCase(),
      };
    }
  ...
  const canPurchase = useMemo(() => {
    ...
    const hasEnoughSkill = addonData.skillLevel.eq(0)
      ? true
      : skills[SKILL_ENUM[addonData.skill]] &&
        skills[SKILL_ENUM[addonData.skill]].gte(addonData.skillLevel);
            ...
            {canPurchase &&
              ...
              magicBalance && (
                <>
                  ...
                  {bonesAllowance.gte(bonesPrice) &&
                    bonesPrice.gt(0) &&
                    bonesBalance.gte(bonesPrice) && (
                  ...
                  {magicAllowance.gte(magicPrice) &&
                    magicPrice.gt(0) &&
                    magicBalance.gte(magicPrice) && (
