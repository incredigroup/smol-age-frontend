...
const borderColor = (ground: Ground) => {
  switch (ground) {
    ...
    default:
      return 'border-smol-brown-light';
  }
};

export const Stake = () => {
  const { nfts, enterDevelopmentGrounds } = useContext(ContractContext);
  ...
  if (!eligibleSmols || eligibleSmols?.length === 0) {
    return (
      <div className="w-full">
        ...
        <Link
          className="btn mx-auto flex w-fit justify-center py-2"
    ...
    <>
      <div className="grid max-h-[35vh] grid-cols-2 gap-2 overflow-y-scroll xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5">
        {eligibleSmols.map((nft: NFTObject) => {
          ...
          return (
            <button
              //disabled={!isSelected}
              onClick={() => setSelected((prev) => prev?.filter((item) => item.tokenId !== nft.id))}
              key={nft.id.toString()}
              className={cn(
      ...
      <div className="flex h-9 items-center justify-center gap-6">
        ...
        <button
          ...
          onClick={onStake}
          className="flex h-full items-center border-4 border-black bg-smol-brown-alternative px-2 text-[.60rem] uppercase"
        >
          Stake
        </button>
      </div>
