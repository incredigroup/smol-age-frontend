    ...
    <div className="ml-1 flex h-full items-center gap-1">
      <div className="relative">
        <input
          ...
          className="block py-2 pl-2 text-[0.6rem] tracking-tighter text-black outline-none focus:border-smol-brown-light focus:ring-smol-brown-light"
          placeholder="AMOUNT TO STAKE"
          aria-describedby="bone-amount"
        />
        <button
          onClick={() => setNumStake(maxValue)}
          className="absolute inset-y-0 right-0 flex items-center pr-3 text-[0.6rem] text-smol-brown-light"
        >
          MAX
        </button>
      </div>
      <LoadingButton
        ...
        className="btn border-[4px] bg-smol-brown text-[0.55rem]"
      />
    </div>
  );
};
