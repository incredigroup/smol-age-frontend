...
export const ShopComponent = () => {
  ...
  const { mintLaborTools } = useContext(ContractContext);
  ...
  const purchase = async () => {
    if (totalAmount == 0) {
      toast.info('Nothing to purchase');
      return;
    }
  ...
  return (
    <div className="relative min-h-screen bg-[url('/static/images/phase2/shop.gif')] bg-cover bg-center pb-10 max-xs:overflow-x-scroll">
      {address && (
      ...
      <div className="mx-auto w-full max-w-xl self-center px-4 pb-10 pt-10">
        <div className="relative rounded-2xl bg-black/60 px-2 py-6 sm:p-6">
          ...
          {token && (
            <div className="mb-2 flex justify-center">
              <div className="flex w-[70%] flex-col border-4 border-smol-brown-light bg-smol-brown p-3 text-center sm:w-[80%]">
                <span className="mb-2 text-sm">Token price</span>
                <span className="text-xs leading-7">
                  Per tool: {token.pricePerTool}
                  <br />
                  Total: {totalAmount * token.pricePerTool}
                </span>
              </div>
            </div>
          )}
          <div className="mt-5 flex w-full flex-col items-center justify-center gap-6 sm:flex-row">
            ...
            <button
              ...
              className="flex h-9 items-center rounded-xl border-4 border-smol-brown-light bg-smol-brown p-2 px-4 text-[.60rem] uppercase sm:self-end"
            >
