...
export const RemoveBones = ({ selected }: { selected: BigNumber[] }) => {
  const { unstakeBones, isLoading } = useContext(ContractContext);
  ...
  const handleSubmit = () => {
    ...
    unstakeBones(parseUnits(String(bonesToStake), 18), selected?.[1]);
  };
